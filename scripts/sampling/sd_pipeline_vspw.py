...
def sample(
    ...
    device: str = "cuda",
    modulate_block_idx: str = "7",
    modulate_timestep: str = "22",
    feature_timestep: str = "24",
    modulate_schedule: str = "constant",
    modulate_lambda_start: float = 10.0,
    modulate_lambda_end: float = 10.0,
    modulate_layer_type: str = "spatial",
    modulate_attn_type: str = "cross_attn",
    num_masks: int = 10,
    ...
    def load_feature_masks(masks_path, mask_id, num_frames=14, feature_timestep="24", modulate_block_idx=7,
        ...
        for frame_id in range(num_frames):
            ...
            if modulate_block_idx in [0, 1, 2]:
                ...
                resize_height = base_height * 2
                resize_width = base_width * 2
    ...
    def save_feature_maps_callback(i, xt=None):
        if i >= t_start:
            # save_feature_maps(model.model.diffusion_model.output_blocks , i, "input_block", xt=xt)
    ...
    for batch_id in range(batch_start_idx, num_batches):
        ...
        total_frames = latent_video.shape[0]
        H, W = input_video_tensor.shape[2:]
        F = 8
        C = 4
        T = None
        ...
        with torch.no_grad():
            with torch.autocast(device):
                ...
                for k in c:
                    if not k == "crossattn":
                        c[k], uc[k] = map(
                            lambda y: y[k][: math.prod(1)].to("cuda"), (c, uc)
                        )
        ...
        with torch.no_grad():
            with torch.autocast(device):                
                ...
                #######################################################################
                ############################## Step 1. Add nosie to real images
                #######################################################################   
                if inversion_type == "add_noise":
                ...
                if is_injected_features:
                    ...
                    injected_feature_types = [
                        ...
                        "spatial_self_attn_k", "spatial_self_attn_q",
                    ]
                    input_block_indices = [3, 4, 5, 6, 7, 8, 9, 10, 11]
                ...
                for pn in range(2):
                    ...
                    for mask_id in tqdm(ref_unique_labels, desc="modulate mask"):
                        try:
                            ...
                            print(f"Failed to load feature masks for mask {mask_id}: {e}")
                            continue
                        # if mask_id != 8:
                        ...
                        modulate_params = {
                            ...
                            "latent_mask_end": min(modulate_timestep) + 1,
                        }
...
def get_batch(keys, value_dict, N, T, device):
    ...
    for key in keys:
        if key == "txt":
            ...
            print(batch.keys(), value_dict.keys())
...
def load_model_from_config(config, device, ckpt=None, verbose=True):
